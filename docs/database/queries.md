...
- [Query Basics](#query-basics)
    ...
    - [Simplified Queries](#simplified-queries)
- [Working with Database Prefixes Manually](#working-with-database-prefixes-manually)
    - [$db-\>prefixTable()](#db-prefixtable)
    - [$db-\>setPrefix()](#db-setprefix)
    - [$db-\>getPrefix()](#db-getprefix)
- [Protecting Identifiers](#protecting-identifiers)
    - [$db-\>protectIdentifiers()](#db-protectidentifiers)
- [Escaping Values](#escaping-values)
    - [1. $db-\>escape()](#1-db-escape)
    - [2. $db-\>escapeString()](#2-db-escapestring)
    - [3. $db-\>escapeLikeString()](#3-db-escapelikestring)
...
- [Working with Query Objects](#working-with-query-objects)
...
```php
--8<--
database/queries/001.php
--8<--
```

The `query()` method returns a database result **object** when "read" type queries are run which you can use to [show your results](#results). When "write" type queries are run it simply returns true or false depending on success or failure. When retrieving data you will typically assign the query to your own variable, like this:

```php
--8<--
database/queries/002.php
--8<--
```
...
```php
--8<--
database/queries/019.php
--8<--
```

For queries of type "write" it returns true or false, indicating the success or failure of the query. For queries of type "read" it returns a standard [result set](#/database/results).
