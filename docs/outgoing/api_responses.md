...
- [Handling Response Types](#handling-response-types)
...
```php
--8<--
outgoing/api_responses/001.php
--8<--
```
...
```php
--8<--
outgoing/api_responses/002.php
--8<--
```
...
```php
--8<--
outgoing/api_responses/003.php
--8<--
```

This is the array that is used during [Content Negotiation](#/incoming/content-negotiation) to determine which type of response to return. If no matches are found between what the client requested and what you support, the first format in this array is what will be returned.
...
```php
--8<--
outgoing/api_responses/004.php
--8<--
```

So, if your request asks for JSON formatted data in an **Accept** header, the data array you pass any of the `respond*` or `fail*` methods will be formatted by the `CodeIgniter\Format\JSONFormatter` class. The resulting JSON data will be sent back to the client.
...
- **$format** `string` The type of response to return, either `json` or `xml`

This defines the format to be used when formatting arrays in responses. If you provide a `null` value for `$format`, it will be automatically determined through content negotiation.

```php
--8<--
outgoing/api_responses/005.php
--8<--
```
...
The `$data` element can be either a string or an array. By default, a string will be returned as HTML, while an array will be run through json_encode and returned as JSON, unless [Content Negotiation](#/incoming/content-negotiation) determines it should be returned in a different format.
...
!!! note "Note"
    Since it sets the status code and body on the active Response instance, this should always
...
```php
--8<--
outgoing/api_responses/006.php
--8<--
```
...
```php
--8<--
outgoing/api_responses/007.php
--8<--
```
...
```php
--8<--
outgoing/api_responses/008.php
--8<--
```
...
Sets the appropriate status code to use when a command was successfully executed by the server but there is no meaningful reply to send back to the client, typically 204.

```php
--8<--
outgoing/api_responses/009.php
--8<--
```
...
```php
--8<--
outgoing/api_responses/010.php
--8<--
```
...
```php
--8<--
outgoing/api_responses/011.php
--8<--
```
...
```php
--8<--
outgoing/api_responses/012.php
--8<--
```
...
- **$errors** `mixed` The error message or array of messages to show the user.
...
```php
--8<--
outgoing/api_responses/013.php
--8<--
```
...
```php
--8<--
outgoing/api_responses/014.php
--8<--
```
...
```php
--8<--
outgoing/api_responses/015.php
--8<--
```
...
```php
--8<--
outgoing/api_responses/016.php
--8<--
```
...
```php
--8<--
outgoing/api_responses/017.php
--8<--
```
