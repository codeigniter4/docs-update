...
If you don't know HTTP status code for redirection, it is recommended to read <a href="https://developer.mozilla.org/en-US/docs/Web/HTTP/Redirections" target="_blank">Redirections in HTTP</a>.
...
This are handled through the `Cache-Control` and `ETag` headers. This guide is not the proper place for a thorough introduction to all of the cache headers power, but you can get a good understanding over at <a href="https://developers.google.com/web/fundamentals/performance/optimizing-content-efficiency/http-caching" target="_blank">Google Developers</a>.
