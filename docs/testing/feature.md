...
- [Requesting a Page](#requesting-a-page)
    ...
    - [Setting Different Routes](#setting-different-routes)
    - [Setting Session Values](#setting-session-values)
    - [Setting Headers](#setting-headers)
    - [Bypassing Events](#bypassing-events)
    - [Formatting the Request](#formatting-the-request)
    - [Setting the Body](#setting-the-body)
...
3.  The third parameter `$params` accepts an array that is used to populate the superglobal variables for the HTTP verb you are using. So, a method of **GET** would have the **$\_GET** variable populated, while a **POST** request would have the **$\_POST** array populated. The `$params` is also used in `feature-formatting-the-request`.
...
This will also set the <span class="title-ref">Content-Type</span> header for your request accordingly.

```php
--8<--
testing/feature/008.php
--8<--
```
...
This will not set the <span class="title-ref">Content-Type</span> header for you. If you need that, you can set it with the `withHeaders()` method.

## Checking the Response

`FeatureTestTrait::call()` returns an instance of a `TestResponse`. See [Testing Responses](#response) on how to use this class to perform additional assertions and verification in your test cases.
