...
- [Using the Timer](#using-the-timer)
    ...
    - [timer()](#timer)
    - [Timer::record()](#timerrecord)
    - [Viewing Your Benchmark Points](#viewing-your-benchmark-points)
    - [Displaying Execution Time](#displaying-execution-time)
- [Using the Iterator](#using-the-iterator)
    - [Creating Tasks To Run](#creating-tasks-to-run)
...
Alternatively, you can use the [global function](#/general/common-functions) `timer()` to start and stop timers:

```php
--8<--
testing/benchmark/003.php
--8<--
```
...
Since v4.3.0, if you use very small code blocks to benchmark, you can also use the `record()` method. It accepts a no-parameter callable and measures its execution time. Methods `start()` and `stop()` will be called automatically around the function call.

```php
--8<--
testing/benchmark/010.php
--8<--
```
...
```php
--8<--
testing/benchmark/004.php
--8<--
```

You can change the precision of the calculated duration by passing in the number of decimal places you want to be shown as the only parameter. The default value is 4 numbers behind the decimal point:

```php
--8<--
testing/benchmark/005.php
--8<--
```

The timers are automatically displayed in the [Debug Toolbar](#/testing/debugging).
...
```php
--8<--
testing/benchmark/008.php
--8<--
```

Once it has run, it will return an HTML table with the results of the test. If you don't want the results, you can pass in `false` as the second parameter:

```php
--8<--
testing/benchmark/009.php
--8<--
```
