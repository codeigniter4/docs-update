...
CodeIgniter supports Entity classes as a first-class citizen, while keeping them completely optional to use. They are commonly used as part of the Repository pattern, but can be used directly with the [Model](#/models/model) if that fits your needs better.

- [Entity Usage](#entity-usage)
    - [Create the Entity Class](#create-the-entity-class)
    - [Create the Model](#create-the-model)
    - [Working with the Entity Class](#working-with-the-entity-class)
    - [Filling Properties Quickly](#filling-properties-quickly)
    - [Bulk Accessing Properties](#bulk-accessing-properties)
- [Handling Business Logic](#handling-business-logic)
    - [Special Getter/Setter](#special-gettersetter)
- [Data Mapping](#data-mapping)
- [Mutators](#mutators)
    - [Date Mutators](#date-mutators)
    - [Property Casting](#property-casting)
- [Checking for Changed Attributes](#checking-for-changed-attributes)
...
By default, the Entity class will convert fields named <span class="title-ref">created_at</span>, <span class="title-ref">updated_at</span>, or <span class="title-ref">deleted_at</span> into [Time](#/libraries/time) instances whenever they are set or retrieved. The Time class provides a large number of helpful methods in an immutable, localized way.

You can define which properties are automatically converted by adding the name to the `$dates` property:

```php
--8<--
models/entities/010.php
--8<--
```
...
```php
--8<--
models/entities/013.php
--8<--
```

```php
--8<--
models/entities/014.php
--8<--
```
...
```php
--8<--
models/entities/020.php
--8<--
```

```php
--8<--
models/entities/021.php
--8<--
```

!!! note "Note"
    If the casting type is marked as nullable `?bool` and the passed value is not null, then the parameter with the value `nullable` will be passed to the casting type handler. If casting type has predefined parameters, then `nullable` will be added to the end of the list.
