...
- [Auto Routing (Improved)](#auto-routing-improved)
    - [Let's try it: Hello World!](#lets-try-it-hello-world)
    - [Methods](#methods)
    - [Passing URI Segments to Your Methods](#passing-uri-segments-to-your-methods)
    - [Default Controller](#default-controller)
    - [Default Method Fallback](#default-method-fallback)
    - [Organizing Your Controllers into Sub-directories](#organizing-your-controllers-into-sub-directories)
    - [Translate URI To CamelCase](#translate-uri-to-camelcase)
- [Auto Routing (Legacy)](#auto-routing-legacy)
    - [Let's try it: Hello World! (Legacy)](#lets-try-it-hello-world-legacy)
    - [Methods (Legacy)](#methods-legacy)
    - [Passing URI Segments to Your Methods (Legacy)](#passing-uri-segments-to-your-methods-legacy)
    - [Default Controller (Legacy)](#default-controller-legacy)
    - [Organizing Your Controllers into Sub-directories (Legacy)](#organizing-your-controllers-into-sub-directories-legacy)
...
A Controller is simply a class file that handles a HTTP request. [URI Routing](#routing) associates a URI with a controller. It returns a view string or `Response` object.
...
The application's main [Request Instance](#/incoming/incomingrequest) is always available as a class property, `$this->request`.

### Response Object

The application's main [Response Instance](#/outgoing/response) is always available as a class property, `$this->response`.

### Logger Object

An instance of the [Logger](../general/../general/logging.md) class is available as a class property, `$this->logger`.
...
```php
--8<--
incoming/controllers/003.php
--8<--
```

!!! note "Note"
    A number of [time-based constants](#/general/common-functions) are always available for you to use, including `YEAR`, `MONTH`, and more.
...
To simplify data checking, the controller also provides the convenience method `validateData()`.
...
The [Validation Library docs](#/libraries/validation) have details on rule and message array formats, as well as available rules:

```php
--8<--
incoming/controllers/006.php
--8<--
```
...
!!! important "Important"
    This method exists only for backward compatibility. Do not use it in new projects. Even if you are already using it, we recommend that you use the `validateData()` method instead.
...
Internally, this uses the controller's `$this->request` instance to get the data to be validated.

The [Validation Library docs](#/libraries/validation) have details on rule and message array formats, as well as available rules:

```php
--8<--
incoming/controllers/004.php
--8<--
```

!!! warning "Warning"
    When you use the `validate()` method, you should use the [getValidated()](#validation-getting-validated-data) method to get the validated data. Because the `validate()` method uses the [Validation::withRequest()](#validation-withrequest) method internally, and it validates data from [$request-\>getJSON()](#incomingrequest-getting-json-data) or [$request-\>getRawInput()](#incomingrequest-retrieving-raw-data) or [$request-\>getVar()](#incomingrequest-getting-data), and an attacker could change what data is validated.

!!! note "Note"
    The [$this-\>validator-\>getValidated()](#validation-getting-validated-data) method can be used since v4.4.0.

If you find it simpler to keep the rules in the configuration file, you can replace the `$rules` array with the name of the group as defined in **app/Config/Validation.php**:

```php
--8<--
incoming/controllers/005.php
--8<--
```
...
Since v4.2.0, the auto-routing is disabled by default. To use it, see `enabled-auto-routing-improved`.
...
!!! important "Important"
    ...
    Since v4.5.0, if you enable the `$translateUriToCamelCase` option, you can use CamelCase classnames. See `controller-translate-uri-to-camelcase` for details.
...
```php
--8<--
incoming/controllers/011.php
--8<--
```

!!! note "Note"
    Since v4.5.0, if you enable the `$translateUriToCamelCase` option, you can use CamelCase classnames like above. See `controller-translate-uri-to-camelcase` for details.
...
!!! note "Note"
    ...
    If you want another naming convention you need to manually define it using the [Defined Route Routing](#defined-route-routing). Here is an example based on PSR-4 Autoloader:
    
    ```php
--8<--
incoming/controllers/012.php
--8<--
```
...
For more information, please refer to the `routes-configuration-options` section of the [URI Routing](#routing-auto-routing-improved-configuration-options) documentation.
...
!!! important "Important"
    ...
    Since v4.5.0, if you enable the `$translateUriToCamelCase` option, you can use CamelCase directory names. See `controller-translate-uri-to-camelcase` for details.
...
CodeIgniter also permits you to map your URIs using its [Defined Route Routing](#defined-route-routing)..
...
```php
--8<--
incoming/controllers/019.php
--8<--
```

## Extending the Controller

If you want to extend the controller, see [Basecontroller](../extending/basecontroller.md).
