...
- [Creating a Filter](#creating-a-filter)
    - [Before Filters](#before-filters)
        - [Replacing Request](#replacing-request)
        - [Stopping Later Filters](#stopping-later-filters)
        - [Returning Response](#returning-response)
    - [After Filters](#after-filters)
- [Configuring Filters](#configuring-filters)
    - [app/Config/Filters.php](#appconfigfiltersphp)
        - [$aliases](#aliases)
        - [$required](#required)
        - [$globals](#globals)
        - [$methods](#methods)
...
- [Confirming Filters](#confirming-filters)
...
An exception to the non-empty result rule is the `Request` instance. Returning it in the before filter will not stop the execution but only replace the current `$request` object.
...
```php
--8<--
incoming/filters/002.php
--8<--
```

If a `Response` instance is returned, the Response will be sent back to the client and script execution will stop. This can be useful for implementing rate limiting for APIs. See [Throttler](../general/../libraries/throttler.md) for an example.
...
If you want to specify filter to a specific route, use **app/Config/Routes.php** and see [URI Routing](#applying-filters).

!!! note "Note"
    The safest way to apply filters is to [disable auto-routing](#use-defined-routes-only), and [set filters to routes](#applying-filters).
...
!!! warning "Warning"
    It is recommended that you should always add `*` at the end of a URI in the filter settings. Because a controller method might be accessible by different URLs than you think. For example, when `auto-routing-legacy` is enabled, if you have `Blog::index`, it can be accessible with `blog`, `blog/index`, and `blog/index/1`, etc.
...
To do this, add an array with the `except` key and a URI path (relative to BaseURL) to match as the value alongside the alias:

```php
--8<--
incoming/filters/006.php
--8<--
```
...
You can apply filters to all requests of a certain HTTP method, like POST, GET, PUT, etc. In this array, you would specify the method name in **lowercase**. It's value would be an array of filters to run:

```php
--8<--
incoming/filters/008.php
--8<--
```

!!! note "Note"
    Unlike the `$globals` or the `$filters` properties, these will only run as before filters.
...
This property is an array of filter aliases. For each alias, you can specify `before` and `after` arrays that contain a list of URI path (relative to BaseURL) patterns that filter should apply to:

```php
--8<--
incoming/filters/009.php
--8<--
```
...
```php
--8<--
incoming/filters/012.php
--8<--
```

In this example, when the URI matches `admin/*'`, the array `['admin', 'superadmin']` will be passed in `$arguments` to the `group` filter's `before()` methods. When the URI matches `admin/users/*'`, the array `['users.manage']` will be passed in `$arguments` to the `permission` filter's `before()` methods.

### Filter Execution Order

!!! important "Important"
    Starting with v4.5.0, the order in which filters are executed has changed. If you wish to maintain the same execution order as in previous versions, you must set `true` to `Config\Feature::$oldFilterOrder`.
...
!!! note "Note"
    Prior to v4.5.0, the filters that are specified to a route (in **app/Config/Routes.php**) are executed before the filters specified in **app/Config/Filters.php**. And the After Filters in Route filters and Filters filters execution order were not reversed. See [Upgrading Guide](#upgrade-450-filter-execution-order) for details.

## Confirming Filters

CodeIgniter has the following [command](../general/../cli/spark_commands.md) to check the filters for a route.
...
You can also see the routes and filters by the `spark routes` command, but it might not show accurate filters when you use regular expressions for routes. See [URI Routing](#routing-spark-routes) for details.
...
- `honeypot` =\> [Honeypot](../general/../libraries/honeypot.md)
...
- `pagecache` =\> [PageCache](../general/../general/caching.md)
