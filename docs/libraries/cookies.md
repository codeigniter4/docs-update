...
If setting the `$raw` parameter to `true` this validation will be strictly made. This is because PHP's \[setcookie()](#https://www.php.net/manual/en/function.setcookie.php)\_ and \[setrawcookie()](#https://www.php.net/manual/en/function.setrawcookie.php)\_ will reject cookies with invalid names. Additionally, cookie names cannot be an empty string.
