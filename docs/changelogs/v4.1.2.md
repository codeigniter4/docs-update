...
Fixed \[a bug](#https://github.com/codeigniter4/CodeIgniter4/issues/4116)\_ in `current_url()` that prevented configurations with an `indexPage` from including that value in the return value. Any installations using `App::$indexPage` should expect altered values from `current_url()` and all its dependencies (including Response Testing, Pager, Form Helper, Pager, and View Parser).
...
See the repo's \[CHANGELOG_4.1.md](#https://github.com/codeigniter4/CodeIgniter4/blob/develop/changelogs/CHANGELOG-4.1.md)\_ for a complete list of bugs fixed.
