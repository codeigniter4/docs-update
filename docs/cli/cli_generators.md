...
- [Built-in Generators](#built-in-generators)
    ...
    - [make:test](#maketest)
    - [make:validation](#makevalidation)
- [Scaffolding a Complete Set of Stock Code](#scaffolding-a-complete-set-of-stock-code)
- [GeneratorTrait](#generatortrait)
- [Declaring the Location of a Custom Generator Command Template](#declaring-the-location-of-a-custom-generator-command-template)

## Introduction

All built-in generators reside under the `Generators` group when listed using `php spark list`. To view the full description and usage information on a particular generator, use the command:
...
!!! note "Note"
    If you use `--suffix`, the generated controller name will be like `ProductController`. That violates the Controller naming convention when using [Auto Routing](#controller-auto-routing-improved) (Controller class names MUST start with an uppercase letter and ONLY the first character can be uppercase). So `--suffix` can be used when you use [Defined Routes](#defined-route-routing).
