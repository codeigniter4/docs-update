...
- [Adding CodeIgniter4 to an Existing Project](#adding-codeigniter4-to-an-existing-project)
    - [Installation](#installation)
...
After using the `builds` command be sure to run `composer update` to sync your vendor folder with the latest target build. Then, check the [Upgrading](upgrading.md) and update project files if necessary.
...
And run `composer update` to sync your vendor folder with the latest target build. Then, check the Upgrading Guide (**user_guide_src/source/installation/upgrade\_{version}.rst**) and update project files if necessary.
