...
- [Breaking Changes](#breaking-changes)
    ...
    - [Forge::modifyColumn() and NULL](#forgemodifycolumn-and-null)
...
- Due to a bug fix, the status codes of redirects may be changed. See [ChangeLog v4.3.4](#v434-redirect-status-code) and if the code is not what you want, [specify status codes](#response-redirect-status-code).

### Forge::modifyColumn() and NULL

A bug fix may have changed the NULL constraint in the result of [$forge-\>modifyColumn()](#db-forge-modifyColumn). See [Change Log](#v434-forge-modifycolumn). To set the desired NULL constraint, change `Forge::modifyColumn()` to always specify the `null` key.

Note that the bug may have changed unexpected NULL constraints in previous versions.
