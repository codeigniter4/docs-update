...
- CI4's Controller has a special constructor [initController()](#controller-constructor).

- CI4 provides [Request](#/incoming/incomingrequest) and [Responses](#/outgoing/response) objects for you to work with - more powerful than the CI3-way.

- If you want a base controller (`MY_Controller` in CI3), use **app/Controllers/BaseController.php**.

- Calling `echo` within Controllers, as in CI3, is still supported, but it is recommended that a string or Response object be returned from Controllers.
...
```php
--8<--
installation/upgrade_controllers/ci3sample/001.php
--8<--
```
...
```php
--8<--
installation/upgrade_controllers/001.php
--8<--
```
