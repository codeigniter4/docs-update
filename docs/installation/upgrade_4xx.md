...
There is no 12-step checklist for upgrading. Instead, start with a copy of CodeIgniter 4 in a new project folder, [however you wish to install and use it](#/installation/index), and then convert and integrate your app components. We'll try to point out the most important considerations here.
...
- [General Adjustments](#general-adjustments)
    ...
    - [Core Class Changes](#core-class-changes)
...
- CI4 is still available as a [ready-to-run zip or tarball](#../installation/installing-manual).

- It can also be installed using [Composer](../general/../installation/installing_composer.md).
...
- The `defined('BASEPATH') OR exit('No direct script access allowed');` line is not necessary because files outside the **public** folder are not accessible in the standard configuration. And CI4 no longer defines the constant `BASEPATH`, so remove the line in all files.
...
\- The Auto Routing is disabled by default. You need to [define all routes  
...
- In CodeIgniter 4, models are now located in **app/Models** and you have to add the lines `namespace App\Models;` along with `use CodeIgniter\Model;` right after the opening php tag. The last step is to replace `extends CI_Model` with `extends Model`.

- The views of CodeIgniter 4 have been moved to **app/Views**. Furthermore, you have to change the syntax of loading views from `$this->load->view('directory_name/file_name')` to `echo view('directory_name/file_name');`.

- Controllers of CodeIgniter 4 have to be moved to **app/Controllers**. After that, add `namespace App\Controllers;` after the opening php tag. Lastly, replace `extends CI_Controller` with `extends BaseController`.
...
- [Upgrade Models](upgrade_models.md)
- [Upgrade Views](upgrade_views.md)
- [Upgrade Controllers](upgrade_controllers.md)
...
- Classes are instantiated where needed, and framework components are managed by [Services](../concepts/services.md).

- The [Autoloader](#../concepts/autoloader) automatically handles PSR-4 style class locating, within the `App` (**app** folder) and `CodeIgniter` (i.e., **system** folder) top level namespaces; with Composer autoloading support.
...
- CI4 provides [Factories](../concepts/factories.md) that can load a class and share the instance like `$this->load` in CI3.
...
- [Helpers](../general/../general/helpers.md) are pretty much the same as before, though some have been simplified.
...
- Cookies and Headers you set before calling `redirect()` are not automatically carried over to a `RedirectResponse`. You need to call `withCookies()` or `withHeaders()` manually if you want to send them.
...
- The hook point `pre_controller` and `post_controller` have been removed. Use [Filters](../incoming/filters.md) instead.

- The hook point `display_override` and `cache_override` have been removed. Because the base methods have been removed.
...
- See [Core Classes](../extending/core_classes.md) for details.
...
- [Upgrade Configuration](upgrade_configuration.md)
- [Upgrade Database](upgrade_database.md)
- [Upgrade Emails](upgrade_emails.md)
- [Upgrade Encryption](upgrade_encryption.md)
- [Upgrade File Upload](upgrade_file_upload.md)
- [Upgrade HTML Tables](upgrade_html_tables.md)
- [Upgrade Localization](upgrade_localization.md)
- [Upgrade Migrations](upgrade_migrations.md)
- [Upgrade Pagination](upgrade_pagination.md)
- [Upgrade Responses](upgrade_responses.md)
- [Upgrade Routing](upgrade_routing.md)
- [Upgrade Security](upgrade_security.md)
- [Upgrade Sessions](upgrade_sessions.md)
- [Upgrade Validations](upgrade_validations.md)
- [Upgrade View Parser](upgrade_view_parser.md)
