...
- [How do I know if my install is working?](#how-do-i-know-if-my-install-is-working)
- [I have to include index.php in my URL](#i-have-to-include-indexphp-in-my-url)
- [Only the default page loads](#only-the-default-page-loads)
- [No input file specified](#no-input-file-specified)
- [My app works fine locally but not on the production server](#my-app-works-fine-locally-but-not-on-the-production-server)
- [The tutorial gives 404 errors everywhere :(](#the-tutorial-gives-404-errors-everywhere)
- [What's with an unhelpful "Whoops!" page?](#whats-with-an-unhelpful-whoops-page)
- [CodeIgniter Error Logs](#codeigniter-error-logs)
...
If a URL like `/mypage/find/apple` doesn't work, but the similar URL `/index.php/mypage/find/apple` does, that sounds like your **.htaccess** rules (for Apache) are not set up properly, or the `mod_rewrite` extension in Apache's **httpd.conf** is commented out. See `urls-remove-index-php`.
...
```php
--8<--
installation/troubleshooting/001.php
--8<--
```
...
```php
--8<--
installation/troubleshooting/002.php
--8<--
```
...
For example, when you have **app/Controllers/Product.php**, you must use `Product` as the short classname, not `product`.
...
The solution: use Apache to serve your site, or else the built-in CodeIgniter equivalent, `php spark serve` from your project root.
...
You can see the error in the log file. See [CodeIgniter Error Logs](#codeigniter-error-logs) below.
...
You can adjust the error threshold to see more or fewer messages. See [Logging](#logging-configuration) for details.
