...
- [Breaking Changes](#breaking-changes)
    - [Changes for set() method in BaseBuilder and Model class](#changes-for-set-method-in-basebuilder-and-model-class)
    - [Session DatabaseHandler's Database Table Change](#session-databasehandlers-database-table-change)
    - [CSRF Protection](#csrf-protection)
    - [CURLRequest Header Change](#curlrequest-header-change)
    - [Query Builder Changes](#query-builder-changes)
- [Breaking Enhancements](#breaking-enhancements)
    - [Multiple Filters for a Route](#multiple-filters-for-a-route)
...
The change was introduced in v4.1.2. But due to \[a bug](#https://github.com/codeigniter4/CodeIgniter4/issues/4807)\_, the DatabaseHandler Driver did not work properly.
...
```php
--8<--
installation/upgrade_415/001.php
--8<--
```
...
In the previous version, if you didn't provide your own headers, `CURLRequest` would send the request-headers from the browser. The bug was fixed. If your requests depend on the headers, your requests might fail after upgrading. In this case, add the necessary headers manually. See [CURLRequest Class](#curlrequest-request-options-headers) for how to add.
