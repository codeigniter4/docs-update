...
- [Breaking Changes](#breaking-changes)
    - [current_url() and indexPage](#currenturl-and-indexpage)
    - [Cache Keys](#cache-keys)
    - [BaseConnection::query() Return Values](#baseconnectionquery-return-values)
- [Breaking Enhancements](#breaking-enhancements)
    - [ConnectionInterface::isWriteType() Declaration Added](#connectioninterfaceiswritetype-declaration-added)
...
Due to \[a bug](#https://github.com/codeigniter4/CodeIgniter4/issues/4116)\_ in `current_url()`, the resulting URIs could be incorrect for a project's configuration, most importantly: `indexPage` would *not* be included. Projects using `App::$indexPage` should expect altered values from `current_url()` and all its dependencies (including Response Testing, Pager, Form Helper, Pager, and View Parser). Update your projects accordingly.
...
The `CodeIgniter\Test` namespace has had significant improvements to help developers with their own test cases. Most notably test extensions have moved to Traits to make them easier to pick-and-choose across various test case needs. The `CIDatabaseTestCase` and `FeatureTestCase` classes have been deprecated and their methods moved to `DatabaseTestTrait` and `FeatureTestTrait` respectively. Update your test cases to extend the main test case and use any traits you need. For example:

```php
--8<--
installation/upgrade_412/001.php
--8<--
```
...
```php
--8<--
installation/upgrade_412/002.php
--8<--
```
