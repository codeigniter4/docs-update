...
- <a href="http://codeigniter.com/userguide3/libraries/config.html" target="_blank">Config Documentation CodeIgniter 3.X</a>
...
4.  Now, you have to change the config fetching syntax everywhere you fetch config values. The CI3 syntax is something like `$this->config->item('item_name');`. You have to change this into `config('MyConfig')->item_name;`.
...
```php
--8<--
installation/upgrade_configuration/ci3sample/001.php
--8<--
```
...
```php
--8<--
installation/upgrade_configuration/001.php
--8<--
```
